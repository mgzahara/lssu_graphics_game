...
{
  return (activeTile.getRow() >= 0) and (activeTile.getCol() >= 0) and (board[activeTile.getRow()][activeTile.getCol()].isAdjacent(board[clickRow][clickCol]));
}
...
void Game::handleLeftMouseClick(int mouse_x, int mouse_y)
{
  ...
  if (validLeftMouseClick(mouse_x, mouse_y) and board[clickRow][clickCol].isNotBusy())
    {
	...
	{
	  //play shuffle sound
	  if (Mix_PlayChannel(-1, shuffleSound, 0) == -1)
	    {
	      exit(-1);
	    }
	  SDL_Delay(100);
  ...
  if ((refAbove + refBelow) < 2 and (refLeft + refRight) < 2)
      ...
      if (tileIsSwapping)
	{
	  ...
	  switch (board[swapeeRow][swapeeCol].getMatchStatus())
	    ...
	    case 1: // swapee made a match - me idle
